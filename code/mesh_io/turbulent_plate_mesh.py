...
def read_y_plus_id_list(filename):
    contour_id_list = [i for i in range(137, 274)]
    return np.array(contour_id_list, dtype=int)
...
def read_boundary_id_list(filename):
    ...
    boundary_id_list.extend([[x, x+1] for x in range(136)])
    
    return np.array(boundary_id_list, dtype=int)


class Turbulent_plate_mesh(Template_Mesh):
    ...
    def init_mesh(self, use_cache = True):
        if self.cache_name is None:
            self.cache_name = './mesh_io/Turbulent_plate/init_mesh_cache.pymesh'
        ...
        if self.su2Mesh_dir is None:
            raw_file = './mesh_io/Turbulent_plate/test_mesh.su2'
        else:
            raw_file = os.path.join(self.su2Mesh_dir, 'test_mesh.su2')
